...
if(isset($_POST['uname']) && isset($_POST['password']) && isset($_POST['name']) && isset($_POST['re_password'])){
    ...
    if (empty($uname)) {
        header("Location: signup.php?error=Please enter the Username&$user_data");
        exit();


    }else if (empty($pass)) {
        header("Location: signup.php?error=Please enter the password&$user_data");
        exit();
    }
        ...
        header("Location: signup.php?error=Please enter your name&$user_data");
        exit();
    }
        ...
        if (mysqli_num_rows($result) > 0){
            header("Location: signup.php?error=Username already exist, try another&$user_data");
        exit();
        }else{
